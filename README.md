...
Requirements: Python 3.11+, Node 22.12+, `uv`, and a modern WebGL browser. macOS Apple Silicon was tested; Linux CPU/CUDA installations should work with compatible PyTorch/OpenCV wheels but were not tested. The main reconstruction backend has not been validated on native Windows; use WSL2 for that process.
...
The Sandbox now includes **Pathfinder**: select a start and destination from the doorway lists, or use **Pick start in model** / **Pick destination in model** to click floor surfaces, then **Find path**. Green and orange markers identify the endpoints. Routes use the saved model's floor mesh, walls, furniture, and current transforms, with 20 cm obstacle clearance. Editing or recalibrating the scene clears the previous path. A doorway destination means its reachable approach, not a verified building exit.

These are estimated paths through rendered geometry on one floor; they do not verify real-world access, hazards, or observed free space. Missing floor sections and solid door panels block routing. Stairs, cross-floor connections, and live navigation remain future work. See [routing API and adapter details](backend/app/routing/README.md). Additional demo videos can be added under `TestVideosOfRooms/navigation-demo/` without removing the original two inputs.
...
Fly.io can detect the root `Dockerfile`. That image runs the API and Blender worker in one Machine because Fly volumes cannot be shared between Machines. See `deploy/fly.toml.example` for the required persistent volume, Machine size, and Vercel CORS origin. This arrangement does not retain Compose's network isolation between the API and Blender worker. The API has no user authentication, so deploy it only after adding access controls and setting the Gemini key as a Fly secret, never in Git.
...
## Glasses camera and Arduino recording

The main computer runs the frontend and FastAPI/Gemini/Blender backend as described above. `backend/glassesFiles/tap_stream.py` and `tap_buttons/tap_buttons.ino` run with the USB camera and Arduino on either the **same computer** or a **second computer on the same Wi-Fi**. The glasses script has separate dependencies; it does not need the main backend's Python environment or Gemini credentials. Its recordings remain on the camera computer until the dashboard loads a finished video for reconstruction. The branch's sample recordings are excluded.

On the camera computer, check out the same integrated revision (or copy `backend/glassesFiles/`), flash the Arduino sketch, and start the camera script:

```bash
cd backend/glassesFiles
python3 -m venv .venv
.venv/bin/python -m pip install -r requirements.txt
.venv/bin/python tap_stream.py --camera 1 --rotate ccw --host 0.0.0.0 --port 8080
```

On Windows, use `py -m venv .venv`, `.venv\Scripts\python -m pip install -r requirements.txt`, and `.venv\Scripts\python tap_stream.py --camera 1 --rotate ccw --host 0.0.0.0 --port 8080`. Choose the actual camera index (`0` is also common) and Arduino serial port if auto-detection fails (`--serial COM5` on Windows, or the appropriate device path on macOS/Linux). Windows is supported for this camera script; the main reconstruction backend remains on the main computer. `--rotate` rotates the script's live stream and recordings. The script serves live video before and after recording; tapping sensor 1 or using the dashboard buttons starts and stops a mission. Sensor 2 marks hazards when connected; the sketch enables both sockets by default. Saved missions default to `~/vt26_recordings` on the camera computer.

On the camera computer, open `http://127.0.0.1:8080/` to check its stream and controls. For a second computer, find that computer's Wi-Fi IPv4 address (the script prints a suggested LAN URL when started with `--host 0.0.0.0`), allow inbound TCP port 8080 in its firewall, and check `http://<camera-computer-ip>:8080/status` from the main computer. Use the camera computer's IP, **not** the webcam's USB device name or `127.0.0.1` on the main computer. Keep both computers on a network that permits local device connections. For a single-computer setup, omit `--host 0.0.0.0` and use `http://127.0.0.1:8080`.

In the dashboard, select **Glasses camera**, enter `http://<camera-computer-ip>:8080` in **Glasses stream address**, and press **Start camera**. The browser gets the MJPEG `/stream` preview and samples `/frame.jpg` for the existing RF-DETR detection WebSocket on the main computer. Detections and overlays appear in the dashboard and modeling view. **Rotate 90°** rotates both detector input and preview in quarter turns, keeping boxes aligned. When the script reports camera mode in `/status`, a tap or the dashboard's **Start recording / Stop recording** buttons controls the glasses recording. The finished MP4 loads into **Video input** automatically; review it and press **Reconstruct video** to submit it to the existing Gemini/Blender or other selected processing mode. The mission remains saved on the camera computer. Reconstruction is never submitted automatically.

For a physical two-computer check, confirm `/status` and `/frame.jpg` load from the main computer before recording, tap once to start and again to stop, and confirm the finished file appears in the dashboard without an automatic reconstruction request. Then submit it manually and switch to **Computer webcam** to confirm its live feed and boxes replace the glasses feed. Some browsers cannot preview OpenCV's MP4V recording codec; the original MP4 remains on the camera computer and the reconstruction backend can decode it.

To use the **Computer webcam** with the Arduino button, run `tap_stream.py --camera none` on the computer with the Arduino; add `--host 0.0.0.0` if that is a second computer, and enter its LAN address in the dashboard's **Glasses stream address** before selecting **Computer webcam**. Then start the computer webcam in the dashboard. In this mode the script does not open a camera; each Arduino tap starts or stops the browser's own recording. The dashboard's recording buttons still work. The dashboard reads `/status.camera` and only uses the Arduino taps when it is `false`, so the camera and button-only recording modes cannot both control the same session. Switching sources stops the previous preview and detection polling. If the glasses stream or detector disconnects, check its process and use **Retry detection**.

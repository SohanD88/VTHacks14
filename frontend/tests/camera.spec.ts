...
async function jpegQuadrants(page: Page, jpeg: Buffer) {
  return page.evaluate(async encoded => {
    ...
    return [[.25, .25], [.75, .25], [.25, .75], [.75, .75]].map(([x, y]) => {
      const [red, green, blue] = context.getImageData(x * canvas.width, y * canvas.height, 1, 1).data;
...
test('live camera uses JPEG protocol, overlays detections, survives navigation and stops', async ({ page }) => {
  ...
  const checkWebcamAlignment = async (rotation: 0 | 90) => {
    await expect.poll(() => page.locator('.dashboard-view .live-stage').evaluate((stage, angle) => {
      const video = stage.querySelector('video')!;
      const canvas = stage.querySelector('.detection-overlay') as HTMLCanvasElement;
      if (!video.videoWidth || !video.videoHeight || canvas.dataset.count !== '1') return false;
      const bounds = stage.getBoundingClientRect();
      const quarterTurn = angle === 90;
      const sourceWidth = quarterTurn ? video.videoHeight : video.videoWidth;
      const sourceHeight = quarterTurn ? video.videoWidth : video.videoHeight;
      const fit = Math.min(bounds.width / sourceWidth, bounds.height / sourceHeight);
      const width = sourceWidth * fit, height = sourceHeight * fit;
      const offsetX = (bounds.width - width) / 2, offsetY = (bounds.height - height) / 2;
      const sample = (x: number, y: number) => canvas.getContext('2d')!.getImageData(
        Math.round(x * canvas.width / bounds.width), Math.round(y * canvas.height / bounds.height), 1, 1).data;
      const footage = sample(offsetX + width * .7, offsetY + height * .3);
      const border = sample(offsetX + width * .3, offsetY + height * .2);
      const margin = sample(offsetX / 2, bounds.height / 2);
      return footage[3] > 200 && footage[1] > 30 && border[1] > 100 && border[3] > 100 &&
...
test('Rotate 90° turns browser webcam pixels and keeps boxes over the fitted preview', async ({ page }) => {
  await page.addInitScript(() => {
    navigator.mediaDevices.getUserMedia = async () => {
      const canvas = document.createElement('canvas');
      canvas.width = 800; canvas.height = 600;
      const context = canvas.getContext('2d')!;
      context.fillStyle = '#ff0000'; context.fillRect(0, 0, 400, 300);
      context.fillStyle = '#00ff00'; context.fillRect(400, 0, 400, 300);
      context.fillStyle = '#0000ff'; context.fillRect(0, 300, 400, 300);
      context.fillStyle = '#ffff00'; context.fillRect(400, 300, 400, 300);
      (window as Window & { testCamera?: HTMLCanvasElement }).testCamera = canvas;
  ...
  await page.routeWebSocket('**/api/camera/detect', socket => {
    ...
    socket.onMessage(message => {
      if (typeof message === 'string') { header = JSON.parse(message); return; }
      expect(header).not.toBeNull();
      if (header!.width === 480 && header!.height === 640) rotatedJpeg = Buffer.from(message);
      socket.send(JSON.stringify({ ...header, type: 'detections', processing_ms: 10,
        detections: [{ label: 'blue square', confidence: .99, box: [.05, .05, .45, .45] }] }));
      header = null;
    });
  });
  await page.goto('/');
  await page.getByRole('button', { name: 'Start camera', exact: true }).click();
  await expect(page.getByTestId('live-detections')).toContainText('blue square');
  await page.getByRole('button', { name: 'Rotate camera 90 degrees' }).click();
  await expect.poll(() => rotatedJpeg !== null).toBe(true);
  expect(await jpegQuadrants(page, rotatedJpeg!)).toEqual(['blue', 'red', 'yellow', 'green']);
  const checkPreview = async () => expect.poll(() => page.locator('.dashboard-view .detection-overlay').evaluate(canvasElement => {
    const canvas = canvasElement as HTMLCanvasElement;
    const stage = canvas.parentElement!.getBoundingClientRect();
    const fit = Math.min(stage.width / 600, stage.height / 800);
    const width = 600 * fit, height = 800 * fit;
    const offsetX = (stage.width - width) / 2, offsetY = (stage.height - height) / 2;
    const sample = (x: number, y: number) => {
      const [red, green, blue, alpha] = canvas.getContext('2d')!.getImageData(
        Math.round(x * canvas.width / stage.width), Math.round(y * canvas.height / stage.height), 1, 1).data;
    ...
    const colors = [[.25, .25], [.75, .25], [.25, .75], [.75, .75]]
      .map(([x, y]) => sample(offsetX + x * width, offsetY + y * height));
    const edge = sample(offsetX + .25 * width, offsetY + .05 * height);
    return colors.join(',') === 'blue,red,yellow,green' && edge === 'other' &&
...
test('glasses stream detects objects, rotates in both views, and switches back to the computer', async ({ page }) => {
  let snapshots = 0, connections = 0, label = 'cup', glassesAvailable = true;
  ...
  await page.routeWebSocket('**/api/camera/detect', socket => {
    ...
    socket.onMessage(message => {
      ...
      expect(message[0]).toBe(0xff); expect(message[1]).toBe(0xd8);
      if (header!.width === 480 && header!.height === 640 && rotatedJpeg === null) rotatedJpeg = Buffer.from(message);
      socket.send(JSON.stringify({ ...header, type: 'detections', processing_ms: 42,
        detections: [{ label, confidence: .91, box: [.1, .2, .5, .8] }] }));
      header = null;
    });
  });
  await page.goto('/');
  ...
  await page.route('http://127.0.0.1:8080/**', async route => {
    ...
    if (!glassesAvailable) { await route.abort('failed'); return; }
    await route.fulfill({ status: 200, contentType: 'image/jpeg', body: jpeg,
      headers: { 'Access-Control-Allow-Origin': '*' } });
  });
  const dashboard = page.locator('.dashboard-view');
  await dashboard.getByRole('button', { name: 'Start camera', exact: true }).click();
  await expect(page.getByTestId('live-detections')).toContainText('cup');
  label = 'bottle';
  ...
  await expect(dashboard.locator('.detection-overlay')).toHaveAttribute('data-count', '1');
  expect(snapshots).toBeGreaterThan(0);
  await page.setViewportSize({ width: 900, height: 700 });
  await expect.poll(async () => dashboard.locator('.detection-overlay').evaluate((canvas: HTMLCanvasElement) => {
    const stage = canvas.parentElement!.getBoundingClientRect();
    const scale = canvas.width / stage.width;
    const fit = Math.min(stage.width / 800, stage.height / 600);
    const x = ((stage.width - 800 * fit) / 2 + 800 * fit * .3) * scale;
    const y = ((stage.height - 600 * fit) / 2 + 600 * fit * .2) * scale;
    const pixel = canvas.getContext('2d')!.getImageData(Math.round(x), Math.round(y), 1, 1).data;
    return pixel[1] > 100 && pixel[3] > 100;
  })).toBe(true);
  const rotate = dashboard.getByRole('button', { name: 'Rotate camera 90 degrees' });
  frameSizes.length = 0;
  await rotate.click();
  ...
  await expect.poll(async () => dashboard.locator('.detection-overlay').evaluate((canvas: HTMLCanvasElement) => {
    const stage = canvas.parentElement!.getBoundingClientRect();
    const preview = canvas.parentElement!.querySelector('.glasses-video')!.getBoundingClientRect();
    const fit = Math.min(stage.width / 600, stage.height / 800);
    const expectedWidth = 600 * fit, expectedHeight = 800 * fit;
    const scale = canvas.width / stage.width;
    const x = ((stage.width - expectedWidth) / 2 + expectedWidth * .3) * scale;
    const y = ((stage.height - expectedHeight) / 2 + expectedHeight * .2) * scale;
    const pixel = canvas.getContext('2d')!.getImageData(Math.round(x), Math.round(y), 1, 1).data;
    return Math.abs(preview.width - expectedWidth) < 1 && Math.abs(preview.height - expectedHeight) < 1 &&
      pixel[1] > 100 && pixel[3] > 100;
  })).toBe(true);
  await dashboard.getByRole('button', { name: 'Enter model' }).click();
  await expect(page.locator('.modeler-view .glasses-video')).toBeVisible();
  await expect(page.locator('.modeler-view .detection-overlay')).toHaveAttribute('data-count', '1');
  const modelerRotate = page.locator('.modeler-view').getByRole('button', { name: 'Rotate camera 90 degrees' });
  ...
  await expect(dashboard.locator('.camera-message')).toContainText('Cannot read the glasses camera');
  await expect(dashboard.locator('.detection-overlay')).toHaveAttribute('data-count', '0');
  label = 'cup';
  frameSizes.length = 0;
  ...
  await expect(page.getByTestId('live-detections')).toContainText('cup');
  await expect(rotate).toHaveAttribute('title', 'Current rotation: 90°');
  await expect.poll(() => frameSizes.some(size => {
    const [width, height] = size.split('x').map(Number);
    return width < height;
  })).toBe(true);
  expect(connections).toBe(3);
  await dashboard.getByRole('button', { name: 'Stop camera', exact: true }).click();
  const stoppedAt = snapshots;
  await page.waitForTimeout(700);
  expect(snapshots).toBe(stoppedAt);
});

test('unavailable glasses stream reports an error and the saved address survives reload', async ({ page }) => {
  ...
  const dashboard = page.locator('.dashboard-view');
  await dashboard.getByLabel('Camera source').selectOption('glasses');
  await dashboard.getByLabel('Glasses stream address').fill('http://127.0.0.1:8080/stream');
  await dashboard.getByRole('button', { name: 'Start camera', exact: true }).click();
  await expect(dashboard.locator('.camera-message')).toContainText(/Glasses stream unavailable|Cannot read the glasses camera/);
  await expect(dashboard.getByRole('button', { name: 'Retry detection' })).toBeVisible();
  await dashboard.getByLabel('Camera source').selectOption('computer');
  await expect(dashboard.locator('.camera-message')).not.toContainText('Glasses stream unavailable');
  await page.reload();
  await dashboard.getByLabel('Camera source').selectOption('glasses');
  await expect(dashboard.getByLabel('Glasses stream address')).toHaveValue('http://127.0.0.1:8080/stream');
});

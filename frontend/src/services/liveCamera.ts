...
export interface CameraState {
  ...
  starting: boolean;
  status:
    "standby" | "connecting" | "loading" | "ready" | "reconnecting" | "error";
  message: string;
...
function savedGlassesUrl(): string {
  try {
    return localStorage.getItem(GLASSES_URL_KEY) || DEFAULT_GLASSES_URL;
  } catch {
    return DEFAULT_GLASSES_URL;
  }
}
function initialState(): CameraState {
  return {
    ...
    active: false,
    stream: null,
    glassesUrl: savedGlassesUrl(),
    previewUrl: null,
    sourceError: "",
    frameSize: null,
    starting: false,
    status: "standby",
    message: "Connect a camera to detect objects.",
    detections: [],
    devices: [],
    deviceId: "",
    updatedAt: null,
...
function glassesBaseUrl(value: string): string {
  ...
  if (
    ...
    throw new Error(
      "Enter a glasses address starting with http:// or https://.",
    );
  }
  return url.origin;
}
...
export class LiveCameraSession {
  ...
  stop = () => {
    ...
    this.update({
      ...
      glassesUrl: this.state.glassesUrl,
  ...
  selectSource = (source: CameraSource) => {
    ...
    this.update({
      ...
      message:
        source === "glasses"
          ? "Start the glasses camera stream."
          : "Start your computer webcam.",
    });
  ...
  setGlassesUrl = (value: string) => {
    ...
    if (
      this.state.source === "glasses" &&
      (this.state.active || this.state.starting)
    )
      this.stop();
    this.update({
      glassesUrl: value,
      message: "Start the glasses camera with this address.",
    });
    try {
      localStorage.setItem(GLASSES_URL_KEY, value);
    } catch {
      /* Storage is optional. */
    }
  };
  start = async (deviceId?: string) => {
    ...
    this.update({
      starting: true,
      message:
        this.state.source === "computer"
          ? "Requesting camera permission…"
          : "Connecting to glasses camera…",
    });
    if (this.state.source === "glasses") {
      try {
        ...
        this.update({
          active: true,
          starting: false,
          previewUrl: `${this.glassesBase}/stream?session=${generation}`,
          status: "connecting",
          message: "Connecting to glasses stream and detection server…",
        });
        this.connect();
        ...
        this.update({
          ...
          status: "error",
          message:
            "Enter a valid glasses address, such as http://127.0.0.1:8080.",
        });
      }
      return;
    }
    ...
    try {
      ...
      this.update({
        status: "error",
        message: `Camera unavailable: ${message}`,
      });
    }
  };
  private async refreshDevices(generation: number) {
    try {
      ...
      /* Capture can still work when device enumeration is unavailable. */
    }
  }
  retry = () => {
    ...
    if (this.state.source === "glasses")
      this.update({
        sourceError: "",
        previewUrl: `${this.glassesBase}/stream?retry=${Date.now()}`,
      });
    this.connect();
  };
  previewFailed = () => {
    ...
    this.update({
      sourceError:
        "Glasses stream unavailable. Check that tap_stream.py is running.",
      detections: [],
      updatedAt: null,
      processingMs: null,
    });
  ...
  private connect() {
    ...
    this.readyTimer = setTimeout(() => {
      ...
      this.update({
        status: "error",
        message:
          "Model initialization timed out. Check the backend, then retry detection.",
      });
      socket.close();
    }, 180000);
    socket.onopen = () => {
      if (this.socket === socket)
        this.update({
          status: "loading",
          message:
            "Loading RF-DETR Nano. The first run downloads model weights…",
        });
    };
    socket.onmessage = (event) => {
      ...
      if (data.type === "status") {
        if (data.status === "ready") {
          ...
          this.update({
            status: "ready",
            message: "Detection ready. Hold up a common object.",
          });
        ...
        const detections = Array.isArray(data.detections)
          ? data.detections.filter(isDetection)
          : [];
        this.update({
          detections,
          updatedAt: Date.now(),
          ...
          message: detections.length
            ? `${detections.length} ${detections.length === 1 ? "object" : "objects"} in the latest frame.`
        ...
        this.update({
          detections: [],
          updatedAt: null,
          status: this.fatal ? "error" : this.state.status,
    ...
    socket.onclose = () => {
      ...
      if (this.state.active && !this.fatal) {
        this.update({
          status: "reconnecting",
          message: "Detection server disconnected. Retrying…",
        });
        this.reconnectTimer = setTimeout(() => this.connect(), 2000);
      }
    };
    socket.onerror = () => {
      if (this.socket === socket && !this.fatal)
        this.update({
          message: "Cannot reach the detection server. Retrying…",
        });
    };
  }
  private async sendFrame() {
    ...
    if (
      ...
      return;
    if (this.state.source === "glasses" && Date.now() < this.nextGlassesAttempt)
      return;
    const generation = this.generation;
    ...
    try {
      ...
      if (this.state.source === "computer") {
        ...
        try {
          const response = await fetch(
            `${this.glassesBase}/frame.jpg?t=${Date.now()}`,
            { cache: "no-store", signal: controller.signal },
          );
          if (!response.ok)
            throw new Error(`Glasses snapshot returned ${response.status}`);
          image = await createImageBitmap(await response.blob());
        } finally {
          clearTimeout(timeout);
          if (this.frameAbort === controller) this.frameAbort = null;
        }
        try {
          if (generation !== this.generation) return;
          [width, height] = this.drawFrame(
          ...
          if (
            ...
            this.state.frameSize[0] !== image.width ||
            this.state.frameSize[1] !== image.height
          ) {
            this.update({
              sourceError: "",
      ...
      const blob = await new Promise<Blob | null>((resolve) =>
        this.canvas.toBlob(resolve, "image/jpeg", 0.75),
      );
      if (
        !blob ||
      ...
      this.pendingTimer = setTimeout(() => {
        if (this.pendingId === id) socket.close();
      }, 12000);
    } catch {
      if (generation === this.generation && this.state.source === "glasses") {
        ...
        this.update({
          sourceError:
            "Cannot read the glasses camera. Check its address and tap_stream.py.",
          detections: [],
          updatedAt: null,
          processingMs: null,
        });
      ...
      if (generation === this.generation) this.capturing = false;
    }
  }
  private drawFrame(
...
function isDetection(value: unknown): value is LiveDetection {
  ...
  return (
    ...
    item.box.length === 4 &&
    item.box.every((n) => Number.isFinite(n) && n >= 0 && n <= 1) &&

...
export function Dashboard({ camera, active, scan, busy, error, health, onScan, onOpen, onCheckHealth }: Props) {
  ...
  return <section className={`view dashboard-view ${active ? 'is-active' : ''}`} aria-label="Spatial intelligence dashboard" aria-hidden={!active} inert={!active}>
    ...
    <div className="dashboard-grid">
      <section className="panel camera-panel" aria-labelledby="camera-title"><div className="panel-heading"><div><span className="panel-index">01</span><h2 id="camera-title">Live camera</h2></div><span className="live-badge">{camera.active && !camera.sourceError ? 'Live' : 'Offline'}</span></div><CameraFeed camera={camera} visible={active} /></section>

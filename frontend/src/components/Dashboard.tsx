...
export function Dashboard({ active, scan, busy, error, health, onScan, onOpen, onCheckHealth }: Props) {
  ...
  return <section className={`view dashboard-view ${active ? 'is-active' : ''}`} aria-label="Spatial intelligence dashboard" aria-hidden={!active} inert={!active}>
    ...
    <div className={`scan-feedback ${error ? 'has-error' : ''}`} role={error ? 'alert' : 'status'} aria-live="polite" aria-busy={busy}>
      {error || (busy ? 'Processing the sample capture. Waiting for the reconstruction response…' : scan ? `${scan.message} Scan ${scan.id.slice(0, 8)} · ${scan.stats.objects} objects returned.` : 'Demo mode · Run a scan to populate your scene, detections, and spatial model.')}
      {error && scan && <span> Your previous successful scan is still displayed.</span>}
    </div>
    <div className="dashboard-grid">
      <section className="panel camera-panel" aria-labelledby="camera-title"><div className="panel-heading"><div><span className="panel-index">01</span><h2 id="camera-title">Live camera</h2></div><span className="live-badge">Demo source</span></div><CameraFeed /></section>
      ...
      <section className="panel detections-panel" aria-labelledby="detections-title"><div className="panel-heading compact"><div><span className="panel-index">03</span><h2 id="detections-title">Detections</h2></div><span className="muted">{scan ? `${scan.detections.length} classes · simulated` : 'Awaiting scan'}</span></div>
        <div className="detection-list">{scan ? scan.detections.map(d => <div key={d.kind}><span className={`object-icon ${d.kind}-icon`} /><strong className="capitalize">{d.kind}</strong><em>{String(d.count).padStart(2, '0')}</em><div><i style={{ '--value': `${d.confidence * 100}%` } as CSSProperties} /></div><b>{Math.round(d.confidence * 100)}%</b></div>) : <p className="empty-copy">Object classes and confidence will appear after your first scan.</p>}</div>
      </section>

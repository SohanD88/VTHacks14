...
export function Modeler({ camera, active, scan, onBack }: { camera: LiveCamera; active: boolean; scan?: ScanResponse; onBack(): void }) {
  ...
  return <section className={`view modeler-view ${active ? 'is-active' : ''}`} aria-label="3D modeling environment" aria-hidden={!active} inert={!active}>
    ...
    <div className="modeler-workspace">
      ...
      <aside className="floating-panel feed-float"><div className="float-heading"><span>{camera.active && !camera.sourceError ? 'Live feed' : 'Camera offline'}</span><button onClick={() => setFeedHidden(!feedHidden)} aria-label={feedHidden ? 'Expand camera feed' : 'Minimize camera feed'} aria-expanded={!feedHidden}>{feedHidden ? '+' : '−'}</button></div>{!feedHidden && <CameraFeed camera={camera} compact visible={active} />}</aside>
